...
// Việt Nam không có giờ mùa hè, luôn lệch UTC đúng 7 tiếng — quy đổi tay
// thay vì dựa vào timezone của server (Vercel serverless chạy UTC) để giờ
// trong ngày (dùng so với giờ ca chuẩn) và việc gộp log theo NGÀY luôn đúng
// giờ Việt Nam bất kể server đặt ở đâu.
...
// Đơn từ lưu ngày dạng chuỗi tự do do LLM tách ra (thường dd/mm/yyyy) — parse
// phòng thủ, không parse được thì bỏ qua ngày đó, không chặn cả API.
...
export async function GET(req: NextRequest) {
  ...
  // Ngày nào có đơn "xin nghỉ" đã được duyệt -> đánh dấu Nghỉ, không tính
  // thiếu công dù không có log chấm công.
  ...
  // Gom log theo ngày VN: lấy check_in ĐẦU TIÊN và check_out CUỐI CÙNG trong
  // ngày làm mốc giờ vào/ra thực tế của ngày đó.
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (entry && (entry.checkInMin !== null || entry.checkOutMin !== null)) {
      ...
      if (checkIn !== null && checkOut !== null && standardStart !== null && standardEnd !== null && standardMinutes !== null) {
        ...
        // Chưa cấu hình ca nào -> không có gì để so sánh, coi như đủ công
        // (chỉ để hiển thị tạm, admin cần vào Ca làm việc cấu hình sớm).
        status = 'du_cong'
      }
    ...
    // Ngày đã qua, không có log, không có đơn nghỉ được duyệt -> vắng không
    // rõ lý do, tính vào thiếu công.
